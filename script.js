...
async function falarComVozDoKalango(texto, isModoVoz = false) {
    ...
    try {
        ...
        kalangoAudioAtual = new Audio(`https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(textoLimpo)}&tl=pt-BR&client=tw-ob`); kalangoAudioAtual.play();
    }
}
...
function adicionarAoCarrinho(produto, preco, mercado, imagem = '') {
    ...
    salvarCarrinho(); mostrarNotificacao(`Adicionado!`); atualizarContadorCarrinho();
}
...
function renderizarCarrinho() { 
    ...
    carrinho.forEach(i => { 
        total += i.preco * i.qtd; 
        c.innerHTML += `
        <div class="app-card p-3 mb-2 flex justify-between items-center animate-fade-in border border-slate-100">
            <div class="flex items-center gap-3 w-2/3">
                <div class="produto-img-wrapper !w-12 !h-12 !min-w-[48px] !rounded-lg"><img src="${i.imagem}" onerror="this.src='${FallbackImage}'"></div>
                <div class="truncate">
                    <b class="text-slate-800 text-sm truncate block w-full">${i.produto}</b>
                    <small class="text-slate-500 text-[10px] block">${i.mercado}</small>
                </div>
            </div>
            <div class="text-right">
                <span class="text-emerald-600 font-bold block text-base mb-1">R$ ${(i.preco * i.qtd).toFixed(2)}</span>
                <div class="flex items-center justify-end gap-2 bg-slate-50 rounded-lg border border-slate-100 p-0.5">
                    <button onclick="alterarQtd('${i.id}',-1)" class="text-slate-400 font-bold hover:text-red-500 px-2">-</button> 
                    <span class="text-slate-800 font-bold text-sm w-4 text-center">${i.qtd}</span> 
                    <button onclick="alterarQtd('${i.id}',1)" class="text-slate-400 font-bold hover:text-emerald-500 px-2">+</button>
                </div>
            </div>
        </div>`; 
    }); 
...
function atualizarListaCatalogo(d) { 
    ...
    d.forEach(i => { 
        l.innerHTML += `
        <div class="app-card p-3 mb-3 flex justify-between items-center animate-fade-in gap-3">
            ...
            <div class="flex-1 flex justify-between items-center overflow-hidden">
                <div class="w-[55%] pr-2">
                    <b class="text-slate-800 text-sm block truncate mb-0.5">${i.produto}</b>
                    <span class="text-[10px] text-slate-500 font-bold bg-slate-100 px-1.5 py-0.5 rounded">${i.mercado}</span>
                </div>
                <div class="text-right shrink-0">
                    <span class="text-emerald-600 font-black block text-xl mb-1">R$ ${i.preco.toFixed(2)}</span>
                    <button onclick="adicionarAoCarrinho('${i.produto.replace(/'/g, "\\'")}',${i.preco},'${i.mercado.replace(/'/g, "\\'")}', '${i.imagem}')" class="bg-emerald-50 text-emerald-600 font-bold px-3 py-1 rounded-lg text-xs w-full hover:bg-emerald-100">+ Add</button>
                </div>
            </div>
        </div>`; 
    }); 
