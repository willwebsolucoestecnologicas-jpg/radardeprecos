// script.js - v21.0 (Login Oficial + Chat Estável)
// ⚠️ IMPORTANTE: Se você gerar uma nova implantação no Google Apps Script, atualize o link abaixo:
...
// --- CONFIGURAÇÃO DO FIREBASE (A QUE VOCÊ ACHOU) ---
...
auth.onAuthStateChanged((user) => {
    if (user) {
        ...
        if(userProfile) {
            ...
            if(nameDisplay) nameDisplay.textContent = user.displayName.split(' ')[0];
            
            const avatar = document.getElementById('user-avatar');
            if(avatar) avatar.src = user.photoURL;
        }
        
        // Preenche campo oculto
        const usernameField = document.getElementById('username');
        if(usernameField) usernameField.value = user.displayName;
...
// --- CHAT: SAUDAÇÃO INICIAL ---
function verificarSaudacaoChat() {
    const area = document.getElementById('chat-messages');
    // Só manda saudação se a área estiver vazia
...
function iniciarGravacaoVoz() {
    ...
    recognition.onstart = function() {
        ...
        mostrarNotificacao("Estou ouvindo...", "sucesso");
    };
    ...
    recognition.onresult = function(event) {
        ...
        // Envia automaticamente após 500ms
        setTimeout(() => {
...
async function enviarMensagemGemini() {
    ...
    try {
        ...
        let resposta = data.resposta || "Oxe, o Kalango ficou mudo.";
        
        // Verifica comando de adicionar ao carrinho (Vindo da IA)
        ...
        if (comandoAdd && comandoAdd[1]) {
            ...
            // Remove o código técnico da resposta visual
...
function renderizarMensagem(texto, remetente) {
    ...
    // Aceita negrito e quebra de linha
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Injeção do botão de microfone (Dinâmico)
